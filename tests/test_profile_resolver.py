from types import SimpleNamespace

from app.autonomy.model_routing import resolve_task_model_profile
from app.autonomy.profiles import normalize_task_profile, resolve_task_profile_by_name


def test_normalize_task_profile_prefers_rss_newspaper():
    assert normalize_task_profile("rss_newspaper") == "rss_newspaper"
    assert normalize_task_profile("news_magazine") == "rss_newspaper"
    assert normalize_task_profile("maintenance") == "maintenance"
    assert normalize_task_profile("morning_briefing") == "briefing"
    assert normalize_task_profile("briefing") == "briefing"
    assert normalize_task_profile("topic_watcher") == "topic_watcher"


def test_resolve_task_profile_aliases_to_rss_newspaper():
    assert resolve_task_profile_by_name("rss_newspaper").name == "rss_newspaper"
    assert resolve_task_profile_by_name("news_magazine").name == "rss_newspaper"
    assert resolve_task_profile_by_name("maintenance").name == "maintenance"
    assert resolve_task_profile_by_name("morning_briefing").name == "briefing"
    assert resolve_task_profile_by_name("briefing").name == "briefing"
    assert resolve_task_profile_by_name("topic_watcher").name == "topic_watcher"


def test_maintenance_model_profile_prefers_small_model():
    task = SimpleNamespace(profile="maintenance")
    user = SimpleNamespace()
    profile = resolve_task_model_profile(task, user)
    assert profile.execution_model == profile.final_synthesis_model
    assert profile.large_retry_enabled is False
