...
from app.market_data_service import get_daily_market_data, get_intraday_market_data
...
@pytest.mark.asyncio
async def test_get_intraday_market_data_renders_csv_and_saves_document():
    user_id = await _seed_user_with_alphavantage_secret()
    payload = {
        "Meta Data": {"2. Symbol": "SPY"},
        "Time Series (60min)": {
    ...
    async with TestSessionLocal() as db:
        with patch("app.api_adapters.alphavantage.fetch_json", new=AsyncMock(return_value=payload)):
            result = await get_intraday_market_data(
                db,
                user_id=user_id,
                symbol="SPY",
                interval="60min",
        ...
        assert result["symbol"] == "SPY"
        assert result["interval"] == "60min"
        assert result["output_format"] == "csv"
        assert "timestamp,open,high,low,close,volume" in result["rendered"]
        assert result["saved_document_name"].endswith(".csv")

        doc = await db.get(Document, int(result["saved_document_id"]))
        assert doc is not None
        assert "intraday" in doc.original_filename
        assert doc.processing_status == "processing"
